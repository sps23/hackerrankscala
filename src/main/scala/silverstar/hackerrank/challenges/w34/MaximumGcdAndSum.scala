...
/**
  ...
  * *
  * NOTE: gcd(x, y) returns the largest integer that divides both  and .
  * *
  * Input Format
  ...
  * *
  * Explanation 0
  * Over all the pairs, if we choose 8 from array A, and 8 from array , we get gcd(8, 8) = 8,
...
object MaximumGcdAndSum {
  ...
  def maximumGcdAndSum(A: Array[Int], B: Array[Int]): Int = {
    ...
    val gcdAndSum: List[(Int, Int)] = cartesianProduct(A.toList, B.toList).map(xy => (gcd(xy._1, xy._2), xy._1 + xy._2))
    gcdAndSum.maxBy(t => (t._1, t._2))._2
  }
}
